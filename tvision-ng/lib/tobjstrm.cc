...
{
    return bp->pubseekoff( std::streampos(0), ios::cur, ios::in );
}
...
ipstream& ipstream::seekg( streamoff off, std::ios::seekdir dir )
{
    objs.removeAll();
...
opstream& opstream::seekp( streamoff pos, std::ios::seekdir dir )
{
#ifndef __UNPATCHED
...
fpbase::fpbase( int f ) : buf( /* FIXME: f */)
{
    pstream::init( &buf );
}

fpbase::fpbase( int f, char *b, int len ) : buf( /* FIXME: f, b, len */)
{
    pstream::init( &buf );
}
...
void fpbase::open( const char *b, int m, int prot )
{
    ...
    else if( buf.open(b, std::ios::in /*FIXME: m, prot*/) )
...
{
    if( buf.pubsetbuf(b, std::streamsize(len)) != NULL )
        clear(ios::goodbit);
    else
        setstate(ios::failbit);
}
