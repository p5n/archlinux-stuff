...
void validate_string(unsigned char *str)
{
...
void screen_add(int color) {                /* FIXME: word wrapping */
    ...
    /* SERGEJ */ validate_string(message);
...
void screen_show(void) {
    ...
    for (n = LINES; n > 0; n--) {
        wattrset(text_win, COLOR_PAIR(status->room->scrn_color[status->room->scrn+n])/* SERGEJ: | A_BOLD */);
    ...
    wattrset(text_win, COLOR_PAIR(status->room->scrn_color[status->room->scrn])/* SERGEJ: | A_BOLD */);
...
void read_str(void) {
    ...
    switch ((ch = wgetch(read_win))) {
        ...
        case KEY_DC:
            string_del(); break;
        case KEY_BACKSPACE:
		case 127: /* SERGEJ: ncurses 5.5 fix */
            string_bs(); break;
